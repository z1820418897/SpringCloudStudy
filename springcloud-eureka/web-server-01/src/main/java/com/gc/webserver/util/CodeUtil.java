...
public class CodeUtil {
    private final String randString = "0123456789";//随机产生只有数字的字符串

    @Value("${CodeKey}")
    public String CODEKEY;//放到session中的key标示
     ...
     * 生成随机图片
     */
    public void getRandcode(HttpServletRequest request, HttpServletResponse response,Long Id) {
        ...
        session.removeAttribute(CODEKEY+Id);
        session.setAttribute(CODEKEY+Id, randomString);
        g.dispose();
